...
istream& operator >> (istream& is, Sinhvien& SV) {
	cout << "Ma SV: "; is >> SV.MaSV;
	cout << "Ten SV: "; is >> SV.TenSV;
	cout << "Diem TB: "; is >> SV.DiemTB;
	return is;
}
//--------------------------------------//
ostream& operator << (ostream& os, Sinhvien& SV) {
	os << "Ma SV: " << SV.MaSV << endl;
	os << "Ten SV: " << SV.TenSV << endl;
	os << "Diem TB: " << SV.DiemTB << endl;
	return os;
}
//--------------------------------------//
...
void SapxepSV(Sinhvien SV[], int SoSV) {
	...
	for(int i = 0; i < SoSV-1; i++)       
        for(int j = 0; j < SoSV-i-1; j++)  
        	if (SV[j] > SV[j+1]) {//Sau khi da cai dat chong toan tu >
        		tmp = SV[j];
                SV[j] = SV[j+1];
                SV[j+1] = tmp;
			}
